class Game extends Phaser.Scene {
    ...
    create() {
        ...
        for (let couche = 1; couche <= 3; couche++) {
            ...
            for (let t = 0; t < 5; t++) {
                ...
                // on rédimensionne le nuage aléatoirement par rapport à sa couche, plus la couche est grande
                // plus le nuage sera aussi grand
        ...
        // Toutes les variables liées au joueur
    ...
    update(time, delta) {
        // On déplace le fond de la map avec l'effet de parallax
        ...
        this.clouds.forEach((cloud) => {
            ...
            // qu'un autre nuage arrive, alorss que c'est le même
            if(cloud.x < 0 - cloud.width) {
                cloud.x = window.config.width;
            }
        });

        // On gère la physique du personnage et ses anims
        ...
        // Si le joueur est au dessus de la map, on l'empêche d'aller plus haut
        if (this.player.y < 50) {
